...
import { CheckCircle2, AlertCircle, Save, Loader2, Plus, Trash2, Info } from "lucide-react";
...
const PersonaConfig = () => {
  ...
  const getTotalWeight = () => 
    categories.reduce((acc, cat) => acc + Number(cat.weight_percentage || 0), 0);
  ...
  const updateCategory = (catPos: number, updatedFields: any) => {
    setCategories(prev =>
      prev.map(cat =>
        cat.position === catPos 
          ? { ...cat, ...updatedFields, weight_percentage: Number(updatedFields.weight_percentage || cat.weight_percentage) }
          : cat
      )
    );
  };
          ...
          const updatedSubcategories = cat.subcategories.map((sub: any) =>
            sub.position === subPos 
              ? { 
                  ...sub, 
                  ...updatedFields,
                  weight_percentage: Number(updatedFields.weight_percentage ?? sub.weight_percentage),
                  level_id: updatedFields.level_id !== undefined 
...
// };
  ...
  const validation = {
    ...
    categoriesValid: categories.every((_, idx) => Math.abs(getCategorySkillTotal(idx) - 100) < 0.01),
    hasName: personaName.trim().length > 0,
  };

  const canSave = validation.totalValid && validation.categoriesValid && validation.hasName;
      ...
      <div className="max-w-6xl mx-auto space-y-6">
        ...
        <div ref={distributionRef} className="space-y-6 mt-6">
                                  ...
                                  <TableRow key={sub.position} className="hover:bg-muted/30 border-b h-9">
                                    ...
                                    <TableCell className="text-center p-0 align-middle">
                                      <Select
                                        ...
                                        onValueChange={(value) => updateSubcategory(cat.position, sub.position, { 
                                          level_id: value
                                        })}
                                      >
          ...
          <div className="pt-3 border-t border-border/50">
                <Label htmlFor="persona-notes" className="text-sm font-medium mb-2 block">
                  Persona Notes
                </Label>
                <Textarea
                  id="persona-notes"
                ...
                <p className="text-xs text-muted-foreground mt-1">
                  Optional: Add context, assumptions, or special considerations for this persona
                </p>
              </div>
        </div>
        ...
        <Dialog open={showSaveDialog} onOpenChange={setShowSaveDialog}>
          <DialogContent>
            ...
            <div className="space-y-4">
              <div>
                <Label htmlFor="persona-name" className="text-sm font-medium mb-2 block">
                  Persona Name
                </Label>
                <Input
                  id="persona-name"

...
import { X, Loader2, AlertCircle, ChevronDown, ChevronUp, FileText } from "lucide-react";
...
const CandidateDetailsSidebar = ({
  ...
  // Fetch candidate documents with React Query
  const {
    data: documentsData,
    ...
    queryKey: ["candidateDocuments", candidate?.candidate_id],
    queryFn: async () => {
      const response = await fetch(
        `${import.meta.env.VITE_API_URL}/api/v1/candidate/${
          candidate.candidate_id
      ...
      if (!response.ok) {
        throw new Error("Failed to fetch candidate documents");
      }

      return response.json();
    },
    enabled: open && !useMockData && !!candidate?.candidate_id && activeTab === "documents",
    staleTime: 30 * 60 * 1000,
    gcTime: 30 * 60 * 1000,
  });
  ...
  const scores = useMockData ? evaluationData?.scores : scoresData?.scores || [];
  const documents = useMockData ? [] : documentsData?.cvs || [];
            ...
            {(useMockData || (!isLoading && !isError)) && scores.length > 0 && (
              ...
              >
                ...
                >
                  ...
                  {isErrorDocuments && (
                    <div className="flex items-center justify-center py-12">
                      <div className="text-center space-y-4 max-w-md">
                        ...
                        <p className="text-sm text-muted-foreground">
                          We couldn't load the documents. Please try again.
                        </p>
                  ...
                  {/* Documents List */}
                  {!isLoadingDocuments && !isErrorDocuments && documents.length > 0 && (
                    <div className="space-y-4">
                      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                        {documents.map((doc, index) => (
                          <div
                            ...
                            // target="noreffer"
                            onClick={()=>{
                              window.open(doc.url || doc.s3_url, '_blank', 'noopener,noreferrer')
                            }}

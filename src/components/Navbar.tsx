...
const NavbarWithSidebar: React.FC = () => {
  ...
  return (
    <>
      ...
      {/* Sidebar */}
      <aside
        className={`fixed top-0 left-0 h-full w-64 bg-slate-800 border-r border-slate-700 z-50 transform transition-transform duration-300 ease-in-out ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'
          }`}
      >
          ...
          <nav className="flex-1 px-3 py-4 overflow-y-auto">
            <ul className="space-y-1">
              ...
              <li>
                ...
                {/* Submenu */}
                <div
                  className={`overflow-hidden transition-all duration-300 ${isUsersSubmenuOpen ? 'max-h-40 opacity-100' : 'max-h-0 opacity-0'
                    }`}
                >
                  <ul className="space-y-1">
                    <li className=''>
